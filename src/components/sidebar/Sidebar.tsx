...
export const Sidebar = memo(function Sidebar() {
  ...
  return (
    <div className="flex h-full w-full flex-col bg-[#0b0e14] font-pixel md:w-64">
      ...
      <div className="flex-1 space-y-1 overflow-y-auto p-1.5 pixel-scroll">
        {filteredAgents.filter(Boolean).map(agent => (
          <div 
            key={agent.id} 
            onClick={() => selectAgent(agent.id)}
            className={`
              p-1.5 border-2 transition-all cursor-pointer flex items-center gap-2
              ${selectedAgentId === agent.id 
                ? 'bg-cyan-900/20 border-cyan-500/50' 
                : 'bg-[#1a1f2c] border-[#2d3748] hover:border-[#4a5568]'}
            `}
          >
            <div className="w-8 h-8 bg-[#0b0e14] border border-[#2d3748] flex items-center justify-center overflow-hidden rounded-full">
              <Image src={agent.avatar} alt="" width={32} height={32} className="h-full w-full object-cover" />
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex justify-between items-center">
                <span className="truncate text-xs font-bold uppercase text-white">{agent.name}</span>
